...
def listen_for_client(cs):
    ...
    while True:
        ...
        # splitting the list into dictionary to know which player wins 
        if len(msg_list) == 2:
            ...
            for k,v in players_dict.items():
                player_values.append(players_dict[k]) # ['R\x1b[39m', 'S\x1b[39m']  

            slicing = slice(1)
            for i in range(len(player_values)):
                val = player_values[i]
                player_values[i] = val[slicing] # ['R', 'S']
            
            # iterates over each of the player's value
...
# evaluates the score for each player in order to send the appropriate message
